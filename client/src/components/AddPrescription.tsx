...
const AddPrescription: React.FC<propsType> = ({
  callback,
  cancelButton,
  caseId,
}) => {
  ...
  const handleAddNote = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await http.post(
        Api_ENdpoints.CASE_PRESCRIPTION.replace(":caseId", caseId),
        {
          prescription: newNoteText,
          image: imageUrl,
        }
      );
      ...
      <form onSubmit={handleAddNote} className="mt-4">
        ...
        <div className="flex gap-3">
          <button
            ...
            disabled={isLoaded}
          >
            {isLoaded ? "Adding Prescription..." : "Add Prescription"}
          </button>
